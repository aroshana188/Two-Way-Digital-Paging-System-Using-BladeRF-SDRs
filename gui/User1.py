...
# If running on the SAME computer as User2, you might need different ports here
# depending on your specific ZMQ setup in GNU Radio.
# Standard assumption: User1 uses 5555/5557, User2 uses 6666/6667 (or similar).
...
class EmergencyDispatchApp:
    ...
    def create_header(self):
        ...
        # TITLE FOR CENTRAL DISPATCH
        title = tk.Label(header_frame, text="🏥 CENTRAL DISPATCH CONSOLE", bg=self.header_bg, fg="white", font=("Arial Black", 16))
        title.pack()
        # ID FOR CENTRAL DISPATCH
        subtitle = tk.Label(header_frame, text="STATUS: ONLINE | DISPATCH ID: 192.168.1.10", bg=self.header_bg, fg="#BBDEFB", font=("Segoe UI", 9))
        subtitle.pack()
    ...
    def log_message(self, text, direction="sent"):
        ...
        # Detect if this is a raw [INFO] string from GNU Radio and reformat it
        # Input:  [INFO] SRC:192.168.1.20 IP:192.168.1.10 MSG:[N] >> hello
        # Output: FROM 192.168.1.20: [N] >> hello
