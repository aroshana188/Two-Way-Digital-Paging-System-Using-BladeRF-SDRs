...
class ARQ(gr.basic_block):
    """
    ARQ Transport Layer (PDU-based, fixed-length frames)
    ----------------------------------------------------
    ...
    def __init__(self, retransmit_delay=0.2, max_retries=5):
        gr.basic_block.__init__(self, name="ARQ", in_sig=[], out_sig=[])
        ...
        # Pending ACKs
        self.pending_ack = {}  # msg_id -> (frame_pdu, timestamp, retries)
        self.next_msg_id = 0

        # Duplicate filter
        self.last_delivered = {}  # (src, dst) -> msg_id
