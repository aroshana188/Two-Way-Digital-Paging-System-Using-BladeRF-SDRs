...
class msgQueue(gr.basic_block):
    """
    Message Queue Block with Priority Handling (PDU-based)
    ------------------------------------------------------
    Maintains 4 queues:
        TX: normal, emergency
        RX: normal, emergency
    Always prioritizes emergency messages.
    ...
    def handle_in_tx(self, msg_pmt):
        if pmt.is_pair(msg_pmt):
            ...
            if pmt.is_u8vector(data_pmt) and len(pmt.u8vector_elements(data_pmt)) >= 3:
                # Determine emergency from first byte of data
    ...
    def tx_loop(self):
        while self.running:
            ...
            if msg is not None:
                # Pass PDU unchanged (meta empty, data includes headers in first 3 bytes)
